...
const CartItems = ({ items, updateQuantity, removeFromCart }) => {
  ...
  return (
    <>
          ...
          <div key={item.id} className="bg-white rounded-lg shadow-md p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <h3 className="text-md font-semibold text-[#064c4f] mb-1">{item.name}</h3>
                <p className="text-sm text-[#00000088] mb-2">${item.price.toFixed(2)}</p>
                
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <button
                      onClick={() => updateQuantity(item.id, Math.max(1, item.quantity - 1))}
                      className="w-7 h-7 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-[#064c4f]"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 12h-15" />
                      </svg>
                    </button>
                    <span className="w-6 text-center text-[#064c4f] font-medium text-sm">
                      {item.quantity}
                    </span>
                    <button 
                      onClick={() => updateQuantity(item.id, Math.min(10, item.quantity + 1))}
                ...
                <div className="mt-2 text-right">
                  <span className="font-bold text-[#064c4f]">
                    ${(item.price * item.quantity).toFixed(2)}
                  </span>
                </div>
              </div>
      ...
      {/* Modal kodu aynı kalacak */}
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center text-[#000000] z-50">
